...
| `/github-sync [ff] [commit]` | `git pull --rebase` してから push する（`ff` で `--ff-only`、`commit` で事前に `git add -A` してコミット） |
| `/github-sync <submodule> [ff] [commit]` | 指定した submodule 内で同じ同期処理を実行する |
...
- `/github-sync commit` は `git add -A` で全変更をステージする。`commit` を指定しない場合はステージもコミットも行わない

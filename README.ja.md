...
| `/github-sync commit [all]` | `--rebase --autostash` で pull し、ローカルの変更をコミットしてから push する（`all` で未追跡ファイルも含める） |
...
- `/github-sync commit` は `--autostash` で pull するため作業ツリーが dirty でも実行できる。pull 後に未マージパスが残っている場合はコミットしない
- `/github-sync <submodule>` は submodule が detached HEAD または `origin` 未設定の場合は実行を拒否し、リモートの自動作成も行わない
- `/github-sync commit` は submodule パスと併用できない。`/github-commit <submodule>` → `/github-sync <submodule>` の2段実行を使う
